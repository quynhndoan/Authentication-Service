...
public:
    ...
    // undefined behavior.
    HashMap(const HashMap& hm);             // Create HashMap
    ~HashMap();                             // Destroy HashMap
    HashMap& operator=(const HashMap& hm);  // Copy HashMap
...
private:
    ...
    unsigned int totalNumberOfKeyValuePairs;
    unsigned int hashCode(const std::string& key) const;
    void createNewBucket(Node** &bucket, unsigned int totalNumberOfBuckets);
    void reHash();
    void deleteBucket(Node** bucket, unsigned int totalNumberOfBuckets);
